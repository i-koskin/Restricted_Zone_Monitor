...
- **✅ ReID** для стабильного трекинга при частичных пропаданиях
- **✅ Интерактивная разметка** запрещенных зон
...
3. Проверка зон - алгоритм "Ray Casting" используется для определения нахождения объектов (центров bbox) в запрещенных зонах;
...
YOLO_MODEL = "yolov8l.pt"        # Доступные модели: n/s/m/l/x
...
```bash
# Использование видеофайла с сохранением результата
