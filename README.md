...
3. Проверка зон - определение нахождения центров bbox в запрещенных зонах
...
MAX_AGE = 30                     # Максимальное количество кадров, которые объект может не появляться, прежде чем трек будет удалён

MIN_HITS = 5                     # Минимальное количество кадров для подтверждения инициализации трека для объекта
